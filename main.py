#!/usr/bin/env python3
"""
SWADE Character Sheet Exporter
===============================
...
Uso:
    python main.py <archivo_personaje.json> [--output <nombre_salida.pdf>]
"""
...
def strip_html(text) -> str:
    ...
    if isinstance(text, dict):
        # Intentar extraer "value" de un dict tipo {"value": "..."}
        text = text.get("value", text.get("name", ""))
    if not isinstance(text, str):
        return str(text) if text else ""
    if not text:
        return ""
    # Primero limpiar enlaces de Foundry VTT
...
def extract_basic_info(data: dict) -> dict:
    ...
    # Linaje / Raza — puede ser {"name": "..."} o un string plano
    ...
    # Avances — puede ser "value" o la longitud de "list"
...
def extract_derived_stats(data: dict) -> dict:
    ...
    # ── Paso ──
    # Puede estar en system.pace.ground o system.stats.speed
    ...
    # ── Bennies ── (puede estar en system.bennies o system.vitals.bennies)
    ...
    # ── Heridas ── (puede estar en system.wounds o system.vitals.wounds)
    ...
    # ── Fatiga ── (puede estar en system.fatigue o system.vitals.fatigue)
    ...
    # ── Moneda ── (puede estar en system.details.currency o system.currency)
...
def extract_skills(data: dict) -> list:
    ...
    # Ordenar: primero por atributo, luego por nombre
...
def extract_powers(data: dict) -> dict:
    ...
    # Puntos de poder — puede haber múltiples pools (general, Arcane, Cleric...)
    ...
    # Detectar trasfondos arcanos desde edges
    arcane_backgrounds = []
    for item in items:
        if item.get("type") == "edge":
    ...
    for item in items:
        if item.get("type") == "power":
            sys = item.get("system", {})
            desc = strip_html(sys.get("description", ""))
            # Skill puede estar en actions.trait
            skill = sys.get("skill", "")
            if not skill:
